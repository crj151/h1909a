...
<body class="gray-bg">
				...
				<div class="input-group">
					...
					<select class="form-control" name="sCid" id = "txt_search_roleid">
						...
						<c:forEach items="${sList }" var="r">
						 	<option value="${r.cCid }">${r.cCname }</option>
						</c:forEach>
                	</select>
				</div>
			 </div>
					...
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>学生性别：</td>
								<td>
								<input type="radio" id="sSex" name="sSex" value="男" aria-required="true" required/>男
								<input type="radio" id="sSex" name="sSex" value="女" aria-required="true" required/>女
								</td>
								<td>&nbsp;&nbsp;</td>
								<td>日期：</td>
								<td><input type="date" id="sCreate" name="sCreate" class="form-control" aria-required="true" required/></td>
							</tr>
							<tr>
								<td>学校：</td>
								<td >
	...
	<!--导入对话框 -->
	<div class="modal fade" id="modal_user_dao" role="dialog" aria-labelledby="modal_user_dao" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					 <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				...
				<div class="modal-body">
							<form id="form_dao" action="dao.htm" method="post" enctype="multipart/form-data">
							<input type="file" id="ex" name="ex">
							</form>
				</div>
				<div class="modal-footer">
				<button type="button" class="btn btn-danger"  id="dao_user_btn">导入</button>
				<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
			</div>
			</div>
		</div>
	...
	<!--导出对话框 -->
	<div class="modal fade" id="modal_user_daochu" role="dialog" aria-labelledby="modal_user_daochu" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					 <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					 <h4 class="modal-title" id="modal_user_daochu_head"> 导出  </h4>
				</div>
				<div class="modal-body">
							导出所有记录？
				</div>
				<div class="modal-footer">
				<button type="button" class="btn btn-danger"  id="daochu_user_btn">导出</button>
				<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
			</div>
			</div>
		</div>
	...
	$(function () {
	    ...
	    //导入弹框
	    var validator = $("#form_dao").validate({
    		submitHandler: function(form){
   		      $(form).ajaxSubmit({
   		    	dataType:"json",
   		    	success: function (data) {
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#bth_dao").click(function(){
	    			$('#modal_user_dao').modal({backdrop: 'static', keyboard: false});
	    			$("#modal_user_dao").show();
	        });
	    	//报表弹框
	    	$("#bth_echarts").click(function(){
	    		...
	    		$.ajax({
	    		    ...
	    		    success:function(res){
	    		    	...
	    		    	var xdata =new 	Array(); 
	    		    	var ydata =new 	Array(); 
	    	    		res.sclist.forEach(function(vo){
                             xdata.push(vo.cname);
	    	...
	    	$("#btn_daochu").click(function(){
	    			$('#modal_user_daochu').modal({backdrop: 'static', keyboard: false});
	    			$("#modal_user_daochu").show();
	        });
	...
	function initEditUser(getSelection){
		...
		$('#sCreate').val(new Date(getSelection.sCreate).Format("yyyy-MM-dd"));
	...
	$("#daochu_user_btn").click(function(){
		$.ajax({
		    url:"daochu.htm",
		    dataType:"json",
		    type:"post",
